...
def ml_prediction_filter(file_path, prob_cutoff):
    ...
    df[["pdb", "chainid"]] = df["id_chain"].str.split("_", expand=True)
...
ml_prediction_filter('merged1_result_uniquegenes_human.xlsx', prob_cutoff = 0.90)
...
df_tm_filter = filter_tm_score('merged1_result_uniquegenes_human.xlsx', tm_threshold = 0.5)
...
count_df = df_tm_filter.groupby('ig_type')['predicted_igtype'].value_counts().unstack(fill_value=0)
