...
for idx in correct_idx[:10]:  # Print first 10 correct samples
    ...
    probabilities= f"{max(all_probs[idx]):.3f}"
...
print(classification_report(all_labels, all_preds, target_names=list(reverse_label_mapping.values()), digits=4, zero_division=0))
...
plt.tight_layout()
plt.savefig("confusion_matrix_jun11.png")
#plt.show()
