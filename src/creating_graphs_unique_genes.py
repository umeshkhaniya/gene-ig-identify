...
expected_node_features = 1320 #  no coordinates # esm 1280 + 40 
...
def create_graph_node_edge(input_excel_file, pdb_file_path, icn3dss_path, contact_file_path, icn3d_interactions, esm2_embedding_file, label_mapping):
    ...
    with h5py.File(esm2_embedding_file, "r") as esmh5:
        ...
        for index, row in input_data_excel.iterrows():
            ...
            pdb, chain = row["pdbid_chain"].split("_")
            ...
            for source_res in selected_residues:
                ...
                node_features = np.hstack((aa_type_onehot, aa_property_onehot, aromatic_onehot, ss_onehot,phi_psi_feature, 
                                             tetrahedral_geometry_features, forward_calpha_features,reverse_calpha_features, embedding))
...
if __name__ == "__main__":
    ...
    contact_file_path = "../input/structure_features_residues_proteome/"
    icn3d_interactions = "../input/icn3d_interactions/"


    esm2_embedding_file = "./unique_genes/esm2_t33_650M_UR50D_unique_genes_embeddings.h5"
    ...
    all_graphs, graph_lookup = create_graph_node_edge("./unique_genes/human_unique_proteome_all_0.4.xlsx", pdb_file_path, icn3dss_file_path, 
    ...
    torch.save(all_graphs, "./unique_genes/uniquegenes_graphs_nocor_humanprot.pt")
    torch.save(graph_lookup, "./unique_genes/uniquegenes_graph_lookup_nocor_humanprot.pt")
